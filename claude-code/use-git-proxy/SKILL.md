...
`agent/skills/use-git-proxy/SKILL.md`. In the ai-sandbox stack it is already baked
into the `claude` image and dropped into `/workspace/.claude/skills/use-git-proxy/`
by the entrypoint, so it auto-loads. To install it elsewhere:
