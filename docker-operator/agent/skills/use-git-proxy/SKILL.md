...
> **This is a trimmed fork of `claude-code/use-git-proxy/SKILL.md`**, the
> untouched copy vendored from the separate git-proxy repository (kept as-is
> so it can be diffed against upstream on the next re-vendor). This fork
> drops two sections that only make sense for a reader who does NOT already
> have this skill installed: "Installing / obtaining this skill" (you already
> have it — the docker-operator's entrypoint baked it in) and the
> operator-only "Operator configuration notes" section, whose one
> agent-relevant point (what a 403 on an expected-reachable repo means) is
> folded into the "Security model" section below instead of being dropped
> outright.
...
```sh
export GIT_PROXY_URL="http://git-proxy:8080"          # git-protocol leg (compose network)
export GIT_PROXY_BROKER_URL="http://git-proxy:8090"   # broker REST leg (may be unset if broker disabled)
export GIT_PROXY_TOKEN="${AGENT_TOKEN}"               # YOUR Bearer token (never the upstream's)
...
> In the ai-sandbox stack the entrypoint already configures `git` globally with
> `url."http://git-proxy:8080/".insteadOf "https://github.com/"` and the matching
> `http.extraHeader`, so ordinary `git clone https://github.com/<owner>/<repo>.git`
> / `git push` / `git fetch` flow through the proxy with no `-c` flag needed. Use
> the explicit `-c "$GIT_PROXY_HEADER"` form only when you bypass `insteadOf`
> (e.g. pointing `git` at the proxy URL directly).
...
least one run failed) | `"success"` (all completed passing) | `"unknown"` (a run in
...
```sh
# Git leg (every fetch/push needs the header unless insteadOf is configured)
git -c "$GIT_PROXY_HEADER" clone [--filter=blob:none] "$GIT_PROXY_URL/<owner>/<repo>.git"
...
## 4. Security model — what to rely on
...
**A 403 on a repo you expected to be reachable** usually means the operator
has not configured a credential profile for it (the proxy denies any
unconfigured repo before ever contacting the upstream). This is an operator
config change, not something you can fix — ask the operator to add a
credential profile for the repo (or, for anonymous read-only access, a
`public_repos` entry); do not retry blindly.